...
async def get_throughput(
    ...
    try:
        ...
        buckets = await es_service.get_throughput_stats(case_ids=filtered_case_ids)
        return [
            {
...
async def get_protocols(
    ...
    try:
        filtered_case_ids = await get_filtered_case_ids(conn, current_user, es_service)
        buckets = await es_service.get_protocol_stats(case_ids=filtered_case_ids)
...
async def get_top_talkers(
    ...
    try:
        filtered_case_ids = await get_filtered_case_ids(conn, current_user, es_service)
        buckets = await es_service.get_top_talkers(case_ids=filtered_case_ids)
